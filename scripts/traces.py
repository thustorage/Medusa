...
sync_cmd = 'python3 scripts/scheduler_ahead.py --batch_size 0 --model_name {} --qps {} --run_time {} > experiments/traces/qps{}/{}_sync 2>&1'
async_cmd = 'python3 scripts/scheduler_ahead.py --batch_size 0 --model_name {} --async_load --qps {} --run_time {} > experiments/traces/qps{}/{}_async 2>&1'
with_cuda_graph_cmd = 'python3 scripts/scheduler_ahead.py --batch_size 0 --model_name {} --fast_start --async_load --persist_cudagraph --load_graph --qps {} --run_time {} > experiments/traces/qps{}/{}_with_cuda_graph 2>&1'
without_cuda_graph_cmd = 'python3 scripts/scheduler_ahead.py --batch_size 0 --model_name {} --qps {} --run_time {} --enforce-eager > experiments/traces/qps{}/{}_without_cuda_graph 2>&1'
...
for qps in all_qps:
  ...
  dir_path = f"experiments/traces/qps{qps}"
...
print(df)
  ...
  #       average_TTFT_pattern = r"Average TTFT: (\d+\.\d+)"
  #       average_ATT_pattern = r"Average ATT: (\d+\.\d+)"
  #       average_E2E_pattern = r"Average E2E: (\d+\.\d+)"
    
  #       match = re.search(average_TTFT_pattern, line)
  #       if match:
  #         average_TTFT = float(match.group(1))
        
  #       match = re.search(average_ATT_pattern, line)
  #       if match:
  #         average_ATT = float(match.group(1))
          
  #       match = re.search(average_E2E_pattern, line)
  #       if match:
  #         average_E2E = float(match.group(1))
          
  #       if average_TTFT:
  #         data[mode]["average_TTFT"] = average_TTFT
  #       if average_ATT:
  #         data[mode]["average_ATT"] = average_ATT
  #       if average_E2E:
  #         data[mode]["average_E2E"] = average_E2E
