...
def process_requests(engine: LLMEngine,
    ...
    while not recv_q.empty() or engine.has_unfinished_requests():
        ...
        if(not recv_q.empty()):
            ...
            print(req)
            payload = req["payload"]
            prompt_len = max(payload[0]-1, 1)
            prompt = "TO "* (prompt_len)
            prompt = prompt.strip()
    ...
    while True:
        ...
        if(not recv_q.empty()):
            ...
            req = recv_q.get()
            if(req["type"] == "exitack"):
                # drain all requests before exit
                ...
                # can not be commented, otherwise would cause the exitfin msg never received
                print("exiting the worker", flush=True)
                exit(0)
...
def vllm_worker(recv_q, send_q, args, cuda_id, lock):
...
data_points = {}
class Worker:
    def __init__(self, args, cuda_id):
        global worker_id_g
        ...
        self.p = Process(target=vllm_worker, args=(self.send_q, self.recv_q,args,cuda_id,self.lock))
        self.p.start()
        worker_id_g += 1
        self.stop = False
...
def check_and_scale():
    ...
    for i in range(TOTAL_GPUS):
        if(worker_slots[i] is None):
            ...
            if(len(worker_poll) > 0):
                scale_num += 1
                worker_slots[i] = worker_poll[i].pop()
                worker_slots[i].start()
            else:
    ...
    min_onfly_requests = -1
    for i in range(TOTAL_GPUS):
        if(worker_slots[i].stop):
            continue
        if min_onfly_requests == -1:
            min_onfly_requests = worker_slots[i].onfly_requests
        else:
            min_onfly_requests = min(min_onfly_requests, worker_slots[i].onfly_requests)
    for i in range(TOTAL_GPUS):
        if(worker_slots[i].onfly_requests == min_onfly_requests):
            return worker_slots[i]
    assert False
...
if __name__ == '__main__':
    ...
    for i in range(TOTAL_GPUS):
        for _ in range(PRESERVE_PER_GPU):
            worker_poll[i].append(Worker(args,i))
    ...
    for data_point in data_points.values():
        print("print request_id: ", request_id,flush=True)
        request_id += 1
        while len(data_point) < 3:
    ...
    print("Test total time:",end_time-start_time,flush=True)
    ...
    for gpu in range(TOTAL_GPUS):
        for p in worker_poll[gpu]:
            p.p.kill()
    tensor_ops.fini_spdk_daemon()
