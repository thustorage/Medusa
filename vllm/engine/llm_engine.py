...
class LLMEngine:
    ...
    def init_phase1(
        ...
        logger.info(
            ...
            f"log_stats={log_stats}, "
            f"fast_start={model_config.fast_start}, "
            f"persist_cudagraph={model_config.persist_cudagraph}, "
            f"load_graph={model_config.load_graph}, "
            f"batch_size={model_config.batch_size}, "
            f"model_name={model_config.model_name}, "
            f"async_load={model_config.async_load}, "
            f"log={model_config.log}, "
            f"save_tensor={model_config.save_tensor}, "
            f"batch_step={model_config.batch_step}")
        # TODO(woosuk): Print more configs in debug mode.
        
        # stream = torch.cuda.Stream()
        # torch.cuda.set_stream(stream)
        
        # tensor_ops.set_model_name(model_config.model_name)
        # tensor_ops.init_load_thread()
            
        # pytorch bindcore
        # change tensor_ops.cu and spdk_util.cpp(spdk_init) as well
        core_list = list(range(9, 26))
        # takes no time, if profile shows there is xxx ms, then it is because the delay from the tensor_ops.init_load_thread()
        torch.set_num_threads(len(core_list))
        os.sched_setaffinity(os.getpid(), core_list)
        
        if not model_config.persist_cudagraph and not model_config.load_graph:
            # not trace memory malloc and free in pytorch
            torch.cuda.memory_stop_trace_malloc_free()

        self.fast_start = model_config.fast_start
        self.bs = model_config.batch_size
    ...
    def init_phase2(self):
        tensor_ops.set_model_name(self.model_config.model_name)
        if not self.model_config.save_tensor:
            tensor_ops.init_load_thread()
        ...
        if not self.fast_start:
            self._run_workers("load_model_weights")
    ...
    def stop(self):
        pass
        # self.profiler.stop()
        # self.profiler.write_html(self.log, timeline=False, show_all=True)
    
    def get_avg_forward_duration(self):
        return self._run_workers("get_avg_forward_duration")
    ...
    def _init_tokenizer(self, **tokenizer_init_kwargs):
        ...
        if self.model_config.persist_cudagraph and not self.model_config.load_graph:
            # offline phase
            init_kwargs.update(tokenizer_init_kwargs)
            self.tokenizer: TokenizerGroup = TokenizerGroup(
                self.model_config.tokenizer, **init_kwargs)
            pickle.dump(self.tokenizer, open(f"/mnt/memfs/model_tokenizers/{self.model_config.model_name}.pkl", "wb"))
            return
        if self.model_config.fast_start:
            self.tokenizer = pickle.load(open(f"/mnt/memfs/model_tokenizers/{self.model_config.model_name}.pkl", "rb"))
            return
        init_kwargs.update(tokenizer_init_kwargs)
        self.tokenizer: TokenizerGroup = TokenizerGroup(
            self.model_config.tokenizer, **init_kwargs)
    ...
    def _init_cache(self) -> None:
        ...
        # Adjust based on different models
        num_gpu_blocks = 512
        num_cpu_blocks = 512
